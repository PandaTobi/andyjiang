...
function tournamentHeading() {
  ...
  return '<div class="swiss-title-row"><div><p class="swiss-kicker">' + phase + '</p><h1>' + escapeHtml(state.config.name) + '</h1></div>' +
    '<div class="swiss-title-facts">' + roundFact + "<br>continues until ended</div></div>";
}
...
function setupScreen() {
  const players = state.players.map((player) =>
    '<form class="player-editor" data-form="edit-player" data-player-id="' + player.id + '">' +
      ...
      '<div class="swiss-form-actions"><button class="swiss-button" type="submit">save</button><button class="swiss-button swiss-danger" type="button" data-action="remove-player" data-player-id="' + player.id + '">remove</button></div>' +
    "</form>"
  ).join("");
...
function pairingRows(round, isTiebreak = false) {
  return round.pairings.map((pairing) => {
    ...
    if (pairing.blackId === null) {
      return '<div class="pairing-row bye-row"><span class="pairing-board">bye</span>' + playerLine(white, "Player") + '<strong class="bye-mark">+1 point</strong></div>';
    }
    const black = playerById(state, pairing.blackId);
    return '<div class="pairing-row"><span class="pairing-board">board ' + pairing.board + "</span>" + playerLine(white, "White") + '<span class="pairing-versus">vs</span>' + playerLine(black, "Black") + resultControls(pairing, isTiebreak ? "tiebreak" : "result") + "</div>";
  }).join("");
}
...
function standingsView() {
  ...
  return '<section class="swiss-section" aria-labelledby="standings-heading"><div class="swiss-section-heading"><div><h2 id="standings-heading">Standings</h2><p class="swiss-meta">Rank is based only on Swiss score. Equal scores remain tied.</p></div></div>' +
...
function playersView() {
  ...
  const list = state.players.slice().sort((a, b) => a.seed - b.seed).map((player) => {
    const timing = player.lateEntry ? "entered for round " + player.entryRound : "original field";
    return '<div class="player-static"><span class="swiss-meta">#' + player.seed + '</span><span><strong>' + escapeHtml(player.name) + '</strong><br><span class="swiss-tag">' + timing + '</span> ' + (!player.active ? '<span class="swiss-tag">withdrawn after round ' + player.withdrawnAfterRound + "</span>" : "") + '</span><span>' + formatRating(player.rating) + '</span><span>' + (player.active && state.status === "active" ? '<button class="swiss-button swiss-danger" type="button" data-action="withdraw-player" data-player-id="' + player.id + '">withdraw</button>' : '<span class="swiss-meta">' + (player.active ? "field closed" : "inactive") + "</span>") + "</span></div>";
  }).join("");
...
function historyView() {
  const rounds = state.rounds.slice().reverse().map((round) => {
    const games = round.pairings.map((pairing) => {
      const white = playerById(state, pairing.whiteId);
      if (pairing.blackId === null) return '<div class="history-game"><span>—</span><span>' + escapeHtml(white.name) + '</span><strong class="history-result">bye</strong><span>+1 point</span></div>';
      const black = playerById(state, pairing.blackId);
      return '<div class="history-game"><span>' + pairing.board + '</span><span>W · ' + escapeHtml(white.name) + '</span><strong class="history-result">' + (pairing.result || "pending") + '</strong><span>B · ' + escapeHtml(black.name) + "</span></div>";
    }).join("");
...
document.addEventListener("submit", (event) => {
  ...
  if (form.dataset.form === "edit-player") safely(() => { updatePlayer(state, form.dataset.playerId, formObject(form)); showNotice("Player updated."); });
});
...
document.addEventListener("click", (event) => {
  ...
  if (action === "new-tournament") {
    ...
    confirmAction("Start a new tournament?", "This permanently removes the saved tournament, every result, and its history from this browser.", "erase and start over", () => {
      state = null; view = "create"; saveState(); clearNotice(); render();
    });
  }
  ...
  if (action === "start-tournament") {
    confirmAction("Start the tournament?", "Seeds will be assigned and Round 1 will be generated. Player details and tournament settings can no longer be edited.", "start tournament", () => safely(() => { startTournament(state); view = "round"; showNotice("Round 1 generated."); }));
  }
